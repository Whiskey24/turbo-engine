-- =============================================================================
-- Migration: Refactor views — move type_slug from asset_types to portfolio_assets
-- Depends on:  20260531000001_fifo_lots_and_multicurrency.sql
--              20260601000000_move_type_slug_to_portfolio_assets.sql
--
-- What this changes:
--   current_holdings  — drop join to asset_types; read type_slug from portfolio_assets
--   realized_pnl      — same
--   unrealized_pnl    — rebuilt because it depends on current_holdings
--   portfolio_summary — rebuilt because it depends on unrealized_pnl
--
-- Drop order respects the dependency chain:
--   portfolio_summary → unrealized_pnl → current_holdings
--   realized_pnl (independent leaf, but drop before recreating current_holdings
--                 to keep the script self-contained and re-runnable)
-- =============================================================================
...
-- current_holdings
--
-- type_slug is now a direct column on portfolio_assets (pa.type_slug).
-- The JOIN to asset_types has been removed — it was only needed for type_slug.
-- -----------------------------------------------------------------------------
...
-- realized_pnl
--
-- type_slug is now read from portfolio_assets (pa.type_slug).
-- The JOIN to asset_types has been removed.
-- -----------------------------------------------------------------------------
