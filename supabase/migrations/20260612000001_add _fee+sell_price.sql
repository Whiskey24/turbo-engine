...
SELECT
    ...
    pa.ticker,
    pa.isin,
    pa.type_slug                                                    AS asset_type,
    pa.nominal_value                                                AS nominal_value,
    ...
    -- Gross sell price recorded on the transaction (same for every lot
    -- matched against the same sell transaction).
    st.price_per_unit                                               AS sell_price_per_unit,

    -- Fee proportional to the quantity matched from this lot:
    --   lot_quantity / sell_quantity * fee
    -- Converted to base currency using the sell transaction's FX rate.
    -- Guards against zero sell quantity to avoid division by zero.
    CASE WHEN st.quantity > 0
         THEN ROUND(
                  st.fee
                  * (lm.quantity_matched / st.quantity)
                  * st.exchange_rate,
              4)
         ELSE 0
    END                                                             AS fee_base,
...
JOIN public.asset_transactions st ON st.id = lm.sell_transaction_id;  -- for sell price & fee
