...
export default function TradingJournalPage() {
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-slate-900 text-slate-100">
                <div className="text-center space-y-4">
                    ...
                    <p className="text-sm font-medium text-slate-400">Auditing historical transactions & market tickers...</p>
                </div>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="p-6 max-w-7xl mx-auto bg-slate-900 min-h-screen text-slate-100">
                <div className="bg-red-950/40 border border-red-900/60 rounded-xl p-6 text-center max-w-xl mx-auto mt-20">
                    <h2 className="text-lg font-semibold text-red-400 mb-2">Data Engine Fault</h2>
                    <p className="text-sm text-slate-400 mb-4">{error}</p>
                    <button onClick={() => window.location.reload()} className="px-4 py-2 bg-red-900 hover:bg-red-800 transition text-white text-xs font-semibold rounded-lg">
                        Retry Sync
                    </button>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800 pb-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mt-1">
                            Multi-asset portfolio sub-ledger tracking performance under global base <span className="font-mono bg-slate-800 text-slate-200 px-1.5 py-0.5 rounded text-xs">{baseCurrency}</span>.
                        </p>
                    </div>

                    {/* Interactive Class Selector Dropdown */}
                    <div className="flex items-center gap-2 self-start sm:self-auto">
                        <label htmlFor="typeFilter" className="text-xs font-semibold uppercase tracking-wider text-slate-400">Asset Class:</label>
                        <select
                            id="typeFilter"
                            value={selectedType}
                            onChange={(e) => setSelectedType(e.target.value)}
                            className="bg-slate-900 border border-slate-800 text-slate-100 text-xs font-medium rounded-lg px-3 py-2 outline-none focus:border-emerald-500 transition cursor-pointer"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
                        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Open Value ({selectedType === "ALL" ? "Combined" : ASSET_TYPE_LABELS[selectedType]})</span>
                        <div className="text-2xl font-bold mt-2 text-slate-50">{formatCurrency(totalCurrentValueBase, baseCurrency)}</div>
                        <p className="text-xs text-slate-500 mt-1">Total Cost Outlay: {formatCurrency(totalCostBase, baseCurrency)}</p>
                    </div>

                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
                        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Unrealized Performance</span>
                        <div className={`text-2xl font-bold mt-2 ${totalUnrealizedBase >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                            {totalUnrealizedBase >= 0 ? "+" : ""}{formatCurrency(totalUnrealizedBase, baseCurrency)}
                        </div>
                        <p className={`text-xs mt-1 font-medium ${totalUnrealizedBase >= 0 ? "text-emerald-500" : "text-rose-500"}`}>
                            {totalUnrealizedPct >= 0 ? "+" : ""}{formatNumber(totalUnrealizedPct)}% ROI
                        </p>
                    </div>

                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
                        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Realized Performance Summary</span>
                        <div className={`text-2xl font-bold mt-2 ${totalRealizedBase >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                            {totalRealizedBase >= 0 ? "+" : ""}{formatCurrency(totalRealizedBase, baseCurrency)}
                        </div>
                        <p className="text-xs text-slate-500 mt-1">Net gains/losses from liquidated entries</p>
                    </div>
                </div>
                                ...
                                <tbody className="divide-y divide-slate-800/60 bg-slate-950/20">
                                    {filteredUnrealized.length === 0 ? (
                                        ...
                                        filteredUnrealized.map((row) => {
                                            ...
                                            // Display more fractional precision for crypto balances
                                            const quantityDecimals = row.asset_type === "CRYPTO" ? 6 : 4;
                                            return (
                                                <tr key={row.asset_id} className="hover:bg-slate-900/40 transition">
                                                    <td className="p-4">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-semibold text-slate-100">{row.asset_name}</span>
                                                        ...
                                                        {row.ticker && <div className="text-xs text-slate-400 font-mono mt-0.5">{row.ticker} {row.isin ? `• ${row.isin}` : ""}</div>}
                                                    </td>
                                                    <td className="p-4 text-right font-mono text-slate-300">{formatNumber(row.quantity_held, quantityDecimals)}</td>
                                                    ...
                                                    <td className={`p-4 text-right font-mono font-semibold ${isPositive ? "text-emerald-400" : "text-rose-400"}`}>
                                                        {isPositive ? "+" : ""}{formatCurrency(row.unrealized_pnl_base, row.base_currency)}
                                                    </td>
                                                    <td className={`p-4 text-right font-mono text-xs ${row.fx_effect >= 0 ? "text-emerald-500/90" : "text-rose-500/90"}`}>
                                                        {row.fx_effect >= 0 ? "▲ +" : "▼ "}{formatCurrency(row.fx_effect, row.base_currency)}
                                                    </td>
                                                    <td className={`p-4 text-right font-mono font-bold ${isPositive ? "text-emerald-400" : "text-rose-400"}`}>
                                ...
                                <tbody className="divide-y divide-slate-800/60 bg-slate-950/20">
                                    {filteredRealized.length === 0 ? (
                                        ...
                                        filteredRealized.map((row) => {
                                            ...
                                            const quantityDecimals = row.asset_type === "CRYPTO" ? 6 : 4;
                                            return (
                                                <tr key={`${row.sell_transaction_id}-${row.lot_id}`} className="hover:bg-slate-900/40 transition">
                                                    <td className="p-4">
                                                        <div className="flex items-center gap-2">
                                                            <span className="font-semibold text-slate-100">{row.asset_name}</span>
                                                            <span className={`text-[10px] uppercase font-bold tracking-wide border px-1.5 py-0.2 rounded ${ASSET_TYPE_BADGES[row.asset_type]}`}>
                                                        ...
                                                        {row.ticker && <div className="text-xs text-slate-400 font-mono mt-0.5">{row.ticker}</div>}
                                                    </td>
                                                    <td className="p-4 text-right font-mono text-slate-300">{formatNumber(row.quantity_sold, quantityDecimals)}</td>
                                                    <td className="p-4">
                                                        <div className="text-slate-300 text-xs font-medium">
                                                            {row.acquired_at} → {row.sold_at}
                                                        </div>
                                                        <div className="text-[10px] text-slate-400 mt-0.5 flex gap-1.5 items-center">
                                                            <span className="bg-slate-800 px-1.5 py-0.5 rounded font-mono">{row.held_days} Days</span>
                                                            {row.is_long_term && (
                                                                <span className="bg-emerald-950 text-emerald-400 border border-emerald-900/50 px-1 py-0.2 rounded font-semibold scale-95 origin-left">
                                                                    Long-Term Horizon
                                                                </span>
                                                            )}
                                                        </div>
                                                    </td>
                                                    <td className="p-4 text-right font-mono text-slate-400">{formatCurrency(row.cost_basis_base, baseCurrency)}</td>
                                                    <td className="p-4 text-right font-mono text-slate-300">{formatCurrency(row.proceeds_base, baseCurrency)}</td>
                                                    <td className={`p-4 text-right font-mono font-semibold ${isPositive ? "text-emerald-400" : "text-rose-400"}`}>
